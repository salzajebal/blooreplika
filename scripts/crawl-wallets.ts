...
const CATEGORY = { id: "a0", name: "지갑", localId: "wallets" };
const START_CHUNK = 6;
...
async function main() {
  console.log(`CRAWLING ${CATEGORY.name} from chunk ${START_CHUNK}...`);
  let totalInserted = 0;
  ...
  console.log(`  Found ${allIds.size} IDs. Processing from chunk ${START_CHUNK}...`);
  const idsArray = Array.from(allIds);
  const CHUNK_SIZE = 100;
  
  for (let i = START_CHUNK * CHUNK_SIZE; i < idsArray.length; i += CHUNK_SIZE) {
    const chunk = idsArray.slice(i, i + CHUNK_SIZE);
    const results = await processWithConcurrency(chunk, (id) => fetchProductDetail(id, CATEGORY.id), 35);
