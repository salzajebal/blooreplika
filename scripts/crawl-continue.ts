...
const CATEGORIES = [
  { id: "40", name: "하의", localId: "bottoms", startChunk: 33 },
...
async function main() {
  ...
  for (const category of CATEGORIES) {
    ...
    for (let i = category.startChunk * CHUNK_SIZE; i < idsArray.length; i += CHUNK_SIZE) {
      ...
      const results = await processWithConcurrency(chunk, (id) => fetchProductDetail(id, category.id), 30);
  ...
  console.log(`\nSESSION COMPLETE! Added ${totalInserted} products`);
  process.exit(0);
}
