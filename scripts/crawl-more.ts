...
const JOBS = [
  { id: "a0", name: "지갑", localId: "wallets", startChunk: 28 },
...
async function main() {
  console.log("CRAWLING REMAINING PRODUCTS...");
  let totalInserted = 0;
  ...
  for (const category of JOBS) {
    ...
    for (let i = category.startChunk * CHUNK_SIZE; i < idsArray.length; i += CHUNK_SIZE) {
      ...
      const results = await processWithConcurrency(chunk, (id) => fetchProductDetail(id, category), 40);
