...
const CATEGORY = { id: "g0", name: "정품", localId: "genuine" };
const START_CHUNK = 3;
...
async function fetchProductDetail(sourceId: string) {
...
async function main() {
  console.log(`CRAWLING ${CATEGORY.name}...`);
  ...
  console.log(`  Found ${allIds.size} IDs. Starting from chunk ${START_CHUNK}...`);
  const idsArray = Array.from(allIds);
  const CHUNK_SIZE = 100;
  let totalInserted = 0;
  
  for (let i = START_CHUNK * CHUNK_SIZE; i < idsArray.length; i += CHUNK_SIZE) {
    const chunk = idsArray.slice(i, i + CHUNK_SIZE);
    const results = await processWithConcurrency(chunk, (id) => fetchProductDetail(id), 50);
