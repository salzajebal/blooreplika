...
const CATEGORIES = [
  { id: "40", name: "하의", localId: "bottoms", startChunk: 9 },
  { id: "70", name: "신발", localId: "shoes", startChunk: 0 },
  { id: "80", name: "악세사리", localId: "accessories", startChunk: 0 },
  { id: "a0", name: "지갑", localId: "wallets", startChunk: 0 },
...
async function fetchProductDetail(sourceId: string, categoryId: string) {
...
async function main() {
  ...
  console.log("CONTINUE CRAWLING REMAINING CATEGORIES");
  console.log("=".repeat(60));
  ...
  for (const category of CATEGORIES) {
    ...
    while (empty < 3 && page <= 200) {
      ...
      if (page % 20 === 0) console.log(`  Page ${page}: ${allIds.size} IDs`);
      page++;
      await delay(15);
    }
    
    console.log(`  Found ${allIds.size} IDs. Processing from chunk ${category.startChunk}...`);
  ...
  console.log(`\n${"=".repeat(60)}`);
  console.log(`SESSION COMPLETE! Added ${totalInserted} products`);
  console.log(`${"=".repeat(60)}`);
