...
- **Mega-menu navigation (bagstyle.site style)**: Full desktop hover dropdown menus for all 15 nav items. 신상품 → month list; 브랜드 → 4-column brand grid; 성별 → 3-level (남성/여성 > category > subcategory); 의류/가방/지갑/신발/쥬얼리 → combined M+W subcategory list; 시계 → brand list; 골프 → 2-level hover (section > subcats); 선글라스/벨트 → simple sub list; 당일배송/할인상품/베스트상품 → quick category links. Mobile accordion menus mirror desktop structure. BAGS_MEN/WOMEN/JEWELRY_MEN/WOMEN all fully populated with complete subcategory lists (ca_ids). `subname` URL parameter filters by subcategory name across both genders. `cat` URL parameter (`filterCategory`) is secondary category filter for special pages.
